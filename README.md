...
loglinter – статический анализатор кода на Go, проверяющий сообщения в вызовах функций логирования на соответствие заданным правилам стиля и безопасности. Анализатор может запускаться как отдельная утилита или интегрироваться в golangci-lint в качестве плагина.
...
Убедитесь, что используется Go версии 1.22 или выше. Выполните:
...
3. Настройте `golangci-lint`. В файле конфигурации (`.golangci.yml`) укажите путь к плагину:
   ```yaml
   linters-settings:
     custom:
       loglinter:
         path: /абсолютный/или/относительный/путь/к/plugin/loglinter.so
...
4. Запустите `golangci-lint run`. Линтер будет выполнять проверки вместе с остальными.
...
```bash
# Проверить пакет в текущем каталоге
loglinter .

# Проверить все пакеты модуля
loglinter ./...
...
После настройки плагина (см. выше) линтер будет автоматически вызываться при запуске `golangci-lint run`. Вывод соответствует формату `golangci-lint`.

## Тестирование

В репозитории реализованы модульные тесты, использующие пакет `golang.org/x/tools/go/analysis/analysistest`. Тесты находятся в файле `analyzer_test.go`, а тестовые данные — в каталоге `testdata/src/`.
...
Ожидается, что все тесты проходят успешно.
...
При необходимости можно исправить сообщения вручную, руководствуясь подсказками анализатора.

---

*Документация соответствует версии анализатора, реализованной в репозитории. Все описанные правила и способы использования поддерживаются в текущей реализации.*